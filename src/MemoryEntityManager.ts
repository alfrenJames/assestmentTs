  ...
  implements IEntityManager
{
  ...
  async read<T = any>(entityType: EntityTypable<T>, id: string): Promise<T> {
  ...
  async find(entityType: string, query: any): Promise<any> {
    const store = this.getStore(entityType)
    const data = Array.from(store.values()).filter((item) =>
      this.matchQuery(item, query)
    )
  ...
  watch(entityType: string, opts: { id: string }): Observable<any> {
  ...
  private notifyWatchers(entityType: EntityTypable, id: string) {
