...
export abstract class DbEntityManager extends BaseEntityManager implements IEntityManager {
  ...
  protected applySerializableOptions(
    entity: any,
    entityType: string,
    direction: 'onSave' | 'onLoad',
  ): any {
    const entityClass = this.findEntityClass(entityType)
    if (!entityClass) {
      throw new Error(`Unknown entity type: ${entityType}`);
    ...
    for (const key of Object.getOwnPropertyNames(instance)) {
      const options: SerializableOptions = Reflect.getMetadata(
  ...
  async populate(entityType: string, entity: any): Promise<any> {
    const fieldsWithRelationMetadata = Object.getOwnPropertyNames(entity).map(
      (key) => {
