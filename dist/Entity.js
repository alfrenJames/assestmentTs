...
export function setWatchContext(ctx) {
    watchContext = ctx;
}
...
export default class Entity {
    ...
    update(_opts, _callback) {
        ...
        const withPatches = produceWithPatches(this.draft, (draft) => {
            // @ts-ignore
            callback(draft);
        });
        const [newDraft, patches] = withPatches;
        if (patches.length > 0) {
            const [newNewDraft, newNewPatches] = produceWithPatches(newDraft, (draft) => {
                // @ts-ignore
                if (!opts.stealth) {
    ...
    constructor(manager, id, data, config) {
        ...
        const proxy = new Proxy(this, {
            set: (target, property, _value) => {
                ...
                const setter = Reflect.getMetadata(EntityFieldMetadata.ENTITY_FIELD_BEFORE_SET, this, property);
                if (setter) {
            ...
            get: (target, property, receiver) => {
                ...
                if (isEntityField) {
                    ...
                    const getter = Reflect.getMetadata(EntityFieldMetadata.ENTITY_FIELD_GETTER, this, property);
                    if (getter) {
                ...
                if (typeof value === "function") {
                    ...
                    if (isEntityFn) {
                        if ("isRPC" in this.manager) {
                            ...
                            return (...args) => {
                                return man.executeRemote(getEntityTypeName(this), this.id, property, args);
                            };
                        }
                        else {
    ...
    async writePendingUpdates() {
        if (this.isDirty) {
            ...
            if (Object.keys(modifiedFields).length) {
                ...
                await this.manager.update(getEntityTypeName(this), this.id, { ...modifiedFields, id: this.id }, this.revisionNumber);
            }
        }
        this.markClean();
    }
    ...
    static temp(classs, fields, manager = new MockEntityManager()) {
